...
struct BookingDetailView: View {
    ...
    /// Whether the confirm-notes alert is shown.
    @State private var showConfirmNotesAlert = false
    /// Title for the confirm-notes alert.
    @State private var confirmNotesTitle = ""
    /// Message for the confirm-notes alert.
    @State private var confirmNotesMessage = ""
    ...
    /// Group info and member list card.
    @ViewBuilder
                ...
                if let group = currentGroup {
                    ...
                    ForEach(group.memberIDs, id: \.self) { memberID in
                        MemberRowView(
                            ...
                            onTap: {
                                selectedMember = userStore.user(by: memberID) ?? AppUser.unknownUser
    ...
    /// Notes card with inline editing, validation, and confirmation alerts.
    @ViewBuilder
    private var notesSection: some View {
        sectionCard {
                ...
                if isEditingNotes {
                    ...
                    HStack(spacing: 12) {
                        ...
                        Button("Save") {
                            ...
                            if trimmed.isEmpty && booking.notes != nil {
                                confirmNotesTitle = "Remove Notes?"
                                confirmNotesMessage = "The existing notes will be permanently removed."
                            } else if booking.notes != nil {
                                confirmNotesTitle = "Update Notes?"
                                confirmNotesMessage = "The existing notes will be updated."
                            } else {
                                confirmNotesTitle = "Add Notes?"
                                confirmNotesMessage = "Notes will be saved to this booking."
                            }
                            showConfirmNotesAlert = true
                        }
                    ...
                    Text(notes).font(.subheadline).foregroundColor(.mutedGray)
                } else {
                    Text("No notes")
            ...
            .alert(confirmNotesTitle, isPresented: $showConfirmNotesAlert) {
                Button("Save", role: booking.notes != nil ? .destructive : nil) { saveNotes() }
                Button("Cancel", role: .cancel) { }
            } message: {
                Text(confirmNotesMessage)
            }
        }
    }
